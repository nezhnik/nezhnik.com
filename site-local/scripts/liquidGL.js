...
(() => {
  ...
  class liquidGLRenderer {
    constructor(snapshotSelector, snapshotResolution = 1.0, rendererOptions = {}) {
      this._autoRecapture = rendererOptions.autoRecapture !== false;
      this._deferSnapshot = rendererOptions.deferSnapshot === true;
    ...
    _initGL() {
      ...
      const fsSource = `
        ...
        uniform float u_centerRefraction;
        uniform float u_lensSpread;
        uniform float u_lensCurve;
        uniform float u_lensConcave;
        uniform float u_bevelFlat;
        uniform float u_bevelApple;
        ...
        void main(){
          ...
          float ayNorm = abs(vy);
          float dist = length(p);
          float centreBlend = smoothstep(0.15, 0.45, dist);
          float capFeatherBase = smoothstep(0.0, max(4.0, bevel_px * 1.15), sdfIn);
          float capAx = smoothstep(0.52, 0.9, axNorm);
          float capFeather = u_bevelApple > 0.5
            ? mix(1.0, capFeatherBase, capAx * 0.9)
            : capFeatherBase;
          vec2 bezelOffset;

          if (u_bevelApple > 0.5) {
            float sdfEps = 1.4;
            float sdf0 = udRoundBox(p_px, b_px, u_radius);
            float sdfX = udRoundBox(p_px + vec2(sdfEps, 0.0), b_px, u_radius);
            float sdfY = udRoundBox(p_px + vec2(0.0, sdfEps), b_px, u_radius);
            vec2 sdfGrad = vec2(sdfX - sdf0, sdfY - sdf0);
            float gradLen = length(sdfGrad);
            vec2 sdfNormal = gradLen > 0.0005 ? normalize(sdfGrad) : vec2(0.0);
            bezelOffset = sdfNormal * offsetAmt * capFeather * 0.92;
          } else if (u_bevelFlat > 0.5) {
            float signY = vy > 0.0 ? 1.0 : -1.0;
            bezelOffset = vec2(0.0, signY * offsetAmt * 0.58 * capFeather);
          } else {
            vec2 dir = dist > 0.001 ? normalize(p) : vec2(0.0);
            bezelOffset = dir * offsetAmt * centreBlend;
          }
          float spread = clamp(u_lensSpread, 0.0, 1.0);
          float curve = clamp(u_lensCurve, 0.0, 1.0);
          float yFall = mix(3.2, 0.75, spread);
          float yPow = mix(2.6, 1.05, curve);
          float yBand = pow(max(0.0, 1.0 - ayNorm * yFall * 2.0), yPow);
          float capStart = mix(0.13, 0.22, spread);
          float capEnd = mix(0.19, 0.27, spread);
          float xCap = 1.0 - smoothstep(capStart, capEnd, axNorm);
          float lensDome = yBand * mix(1.0, xCap, spread * 0.65 + 0.2);
          float concaveMask = 1.0 - pow(max(0.0, lensDome), mix(0.7, 1.15, curve));
          float lensMask = u_lensConcave > 0.5 ? concaveMask : lensDome;
          float lensStrength = u_centerRefraction * lensMask;
          float lensSign = u_lensConcave > 0.5 ? -1.0 : 1.0;
          vec2 domeOffset = vec2(0.0);
          if (u_bevelApple < 0.5) {
            domeOffset = u_bevelFlat > 0.5
              ? vec2(0.0, vy * lensStrength * 0.28 * lensSign * capFeather)
              : vec2(vx * lensStrength * 0.32 * lensSign, vy * lensStrength * 0.22 * lensSign);
          }
          vec2 offset = bezelOffset + domeOffset;

          float tiltRefractionScale = 0.05;
          vec2 tiltOffset = vec2(tan(radians(u_tiltY)), -tan(radians(u_tiltX))) * tiltRefractionScale;

          float lensMagnify = u_magnify;
          if (u_lensConcave > 0.5) {
            lensMagnify = u_magnify * (1.0 + lensDome * u_centerRefraction * 0.06);
          }
          vec2 localUV = (v_uv - 0.5) / max(0.68, lensMagnify) + 0.5;
          ...
          float oobFade = (u_bevelFlat > 0.5 || u_bevelApple > 0.5) ? 0.05 : 0.01;
          float blend = 1.0 - smoothstep(0.0, oobFade, oob);
          float refractMix = blend * ((u_bevelFlat > 0.5 || u_bevelApple > 0.5) ? capFeather : 1.0);
          vec2 sampleUV = mix(mapped, refracted, refractMix);
          ...
          float rimEdge = 1.0 - smoothstep(0.0, bevel_px * 1.05, sdfIn);

          if (u_bevelFlat > 0.5) {
            float edgePreserve = smoothstep(0.0, max(4.5, bevel_px * 0.9), sdfIn);
            final = mix(baseCol, refrCol, edgePreserve);
          } else if (u_bevelApple > 0.5) {
            float axDiag = abs(vx - vy);
            float light135 = clamp(1.0 - axDiag * 1.35, 0.0, 1.0);
            light135 = mix(0.38, 1.0, light135);
            float rimLight = rimEdge * light135;
            vec3 chromaRim = (refrCol.rgb - baseCol.rgb) * rimLight * 0.2;
            final.rgb += chromaRim;
            final.rgb += rimLight * refrCol.rgb * 0.04;
          }

          if (u_saturationBoost > 1.001) {
            float luma = dot(final.rgb, vec3(0.299, 0.587, 0.114));
            final.rgb = mix(vec3(luma), final.rgb, u_saturationBoost);
          }

          float inShape = (u_bevelFlat > 0.5 || u_bevelApple > 0.5)
            ? 1.0 - smoothstep(-1.0, 1.0, dmask)
            : 1.0 - step(0.0, dmask);
      ...
      this.u = {
        ...
        centerRefraction: gl.getUniformLocation(
          this.program,
          "u_centerRefraction"
        ),
        lensSpread: gl.getUniformLocation(this.program, "u_lensSpread"),
        lensCurve: gl.getUniformLocation(this.program, "u_lensCurve"),
        lensConcave: gl.getUniformLocation(this.program, "u_lensConcave"),
        bevelFlat: gl.getUniformLocation(this.program, "u_bevelFlat"),
        bevelApple: gl.getUniformLocation(this.program, "u_bevelApple"),
      ...
      const attemptCapture = async (
        ...
        try {
          ...
          const fullH = Math.max(
            target.scrollHeight,
            target.offsetHeight,
            isFullPage ? document.documentElement.scrollHeight : 0
          );
          this._snapshotDocWidth = fullW;
          this._snapshotDocHeight = fullH;
    ...
    _renderLens(lens) {
      ...
      let isViewportAnchored = false;
      let anchorNode = lens.el;
      while (anchorNode && anchorNode !== document.body) {
        const pos = window.getComputedStyle(anchorNode).position;
      ...
      if (window.visualViewport && !isViewportAnchored) {
      ...
      gl.uniform1f(
        this.u.centerRefraction,
        lens.options.centerRefraction !== undefined
          ? lens.options.centerRefraction
          : 0.0
      );
      gl.uniform1f(
        this.u.lensSpread,
        lens.options.lensSpread !== undefined ? lens.options.lensSpread : 0.8
      );
      gl.uniform1f(
        this.u.lensCurve,
        lens.options.lensCurve !== undefined ? lens.options.lensCurve : 0.5
      );
      gl.uniform1f(
        this.u.lensConcave,
        lens.options.lensConcave ? 1.0 : 0.0
      );
      gl.uniform1f(
        this.u.bevelFlat,
        lens.options.bevelFlat !== false ? 1.0 : 0.0
      );
      gl.uniform1f(
        this.u.bevelApple,
        lens.options.bevelApple ? 1.0 : 0.0
      );
      gl.uniform1f(
        this.u.saturationBoost,
        lens.options.saturationBoost !== undefined ? lens.options.saturationBoost : 1.0
      );
  ...
  class liquidGLLens {
    ...
    setShadow(enabled) {
      ...
      const SHADOW_VAL =
        "0 4px 22px rgba(0,0,0,0.06), 0 0 0 0.5px rgba(0,0,0,0.04)";
  ...
  window.liquidGL = function (userOptions = {}) {
    ...
    if (!renderer) {
      renderer = new liquidGLRenderer(options.snapshot, options.resolution, {
        autoRecapture: options.autoRecapture,
